...
#define DEFAULT_STATUS_HEIGHT 80
...
int video_width, video_height, status_height, tile_height;
float video_scale;
SDL_Surface * screen;
...
int main(int argc, char * argv[])
{
  ...
  /* Allocate dynamic arrays based on video dimensions: */

  auroras = (int *)malloc(video_width * sizeof(int));
  if (auroras == NULL)
    {
      fprintf(stderr, "Error: Could not allocate memory for auroras\n");
      exit(1);
    }

  /* Calculate proportional status bar height and tile height */
  status_height = video_height * DEFAULT_STATUS_HEIGHT / DEFAULT_HEIGHT;
  tile_height = video_height / LAND_HEIGHT;
  ...
  do
    {
      ...
      if (dying[player] == 0 &&
          dancing[player] == 0)
        {
          if (up_down == 1 && y[player] > status_height)
          ...
          if (lasers[i].alive)
            {
              ...
              if (lasers[i].x2 == video_width)
                {
                  lasers[i].x1 = lasers[i].x1 + LASER_SPEED;
                  if (lasers[i].x1 >= video_width)
                    lasers[i].alive = 0;
                }
          ...
          if (bullets[i].alive)
            {
              ...
              if (bullets[i].y < status_height || bullets[i].y >= video_height ||
                  bullets[i].timer <= 0)
                bullets[i].alive = 0;
          ...
          if (explosion_bits[i].alive)
            {
              ...
              if (explosion_bits[i].y < status_height || explosion_bits[i].y >= video_height)
                explosion_bits[i].alive = 0;
              ...
              if (penguinoids[player][i].mode == PENG_MODE_FALLING)
                {
                  ...
                  if (land[player][penguinoids[player][i].x / 32]
                      [penguinoids[player][i].y / tile_height] != -1)
                    {
              ...
              else if (penguinoids[player][i].mode == PENG_MODE_SAVED)
                {
                  ...
                  if (tmp_x >= 0 && tmp_x < LAND_WIDTH)
                    { 
                      if (land[player][tmp_x][penguinoids[player][i].y / tile_height] != -1)
                        {
                          penguinoids[player][i].ym = 0;
                          penguinoids[player][i].mode = PENG_MODE_WALKING;
                          penguinoids[player][i].being_homed = 0;
                      ...
                      if (penguinoids[player][i].y > 448)
                        {
                          ...
                          penguinoids[player][i].ym = -((rand() % 2) + 1);
                               ...
                               [penguinoids[player][i].y / tile_height]
                               == -1)
                        {
                          penguinoids[player][i].y =
              ...
              for (j = 0; j < MAX_LASERS; j++)
                {
                  if (lasers[j].alive && penguinoids[player][i].alive)
                    {
                      if (((lasers[j].x2 == video_width &&
                            tmp_x >= lasers[j].x1 && tmp_x <= video_width) ||
                           (lasers[j].x2 == 0 &&
              ...
              for (j = 0; j < LAND_HEIGHT; j++)
                {
                  if (land[player][i][j] != -1)
                    {
                      add_explosion(i * 32 + 16, j * tile_height + tile_height / 2,
                                    IMG_LASERS);
                      land[player][i][j] = -1;
                    }
                }
              ...
              if (aliens[player][i].type == ALIEN_UFO)
                {
                  ...
                  if (aliens[player][i].mode == ALIEN_MODE_NORMAL)
                    {
                      ...
                      if (land[player][aliens[player][i].x / 32]
                          [(aliens[player][i].y / tile_height) + 1]
                          != -1)
                        {
                      ...
                      if (aliens[player][i].y < status_height)
                        {
                          aliens[player][i].y = status_height;
                          aliens[player][i].ym = 2;
                          aliens[player][i].timer = (rand() % 20) + 20;
                        }
                  ...
                  if ((aliens[player][i].mode == ALIEN_MODE_HOME_PENG &&
                      ...
                      (aliens[player][i].mode == ALIEN_MODE_NORMAL &&
                       ...
                       == 0))
                    {
                      if (abs(x[player] - aliens[player][i].x) <= (video_width / 2))
                        {
              ...
              else if (aliens[player][i].type == ALIEN_MUTANT)
                {
                  ...
                  if (aliens[player][i].mode == ALIEN_MODE_NORMAL &&
                    ...
                    {
                      if (x[player] - aliens[player][i].x >= -(video_width / 2) &&
                          x[player] - aliens[player][i].x <= (video_width / 2))
                        {
              ...
              else if (aliens[player][i].type == ALIEN_BOSS)
                {
                  ...
                  if (aliens[player][i].mode == ALIEN_MODE_NORMAL ||
                  ...
                  else if (aliens[player][i].mode == ALIEN_MODE_DYING)
                    {
                      ...
                      if (aliens[player][i].y >= video_height)
                        {
                          kill_alien(i);
                          flash = 50;
                          flash_colors = 1;
      ...
      if (level_beat[player] > 0)
        {
          ...
          if (level_beat[player] >= 170 - MAX_PENGUINOIDS &&
              (level_beat[player] - (170 - MAX_PENGUINOIDS)) < num_penguinoids)
            {
              ...
              add_points((((video_width - (num_penguinoids * 32)) / 2) +
                          ((level_beat[player] -
                            (170 - MAX_PENGUINOIDS)) * 32) + 16 +
                          x[player] - scroll[player]),
                         status_height + (images[IMG_LEVEL_BONUS] -> h) + 32,
                         IMG_100);
      ...
      else if (scroll[player] < video_width - 128 - 64 && dir[player] == DIR_LEFT)
        scroll[player] = scroll[player] + 16;
...
#define LAND_LEFT ((video_width / 2) - ((LAND_WIDTH / 2) * 4) - 3) // Was 177 on a 640 display
...
#define VIEW_LEFT ((video_width / 2) - ((video_width / 2) / 8))  // Was 280 on a 640 display
      ...
      for (i = 0; i < num_players; i++)
        {
          ...
          for (j = 0; j < lives[i] && j < 11; j++)
            {
              if (lives[i] < 7)
                dest.x = j * 24 + ((video_width - 170) * i);
              else
                dest.x = j * 12 + ((video_width - 170) * i);
          ...
          if (player != i || (frame % 10) < 5)
            draw_number((video_width - 170) * i, 40, score[i]);
          
          
          /* ((level)): */
          draw_number((video_width - 170) * i, 0, level[i]);
        }
      ...
      for (i = 0; i < MAX_STARS; i++)
        {
          ...
          if (dest.x > -31 && dest.x < video_width)
            {
              dest.y = stars[i].y;
              dest.w = j;
              dest.h = 1;
      ...
      */
          ...
          if (aliens[player][i].alive)
            {
              ...
              if (tmp_x >= -(tmp_w - 1) && tmp_x < video_width)
                {
                  /* Which picture? */
              ...
              if (dancing[player] > 200)
                {
                  ...
                  dest.x = (video_width / 2) - 16;
                  dest.y = status_height;
                  dest.w = 32;
      ...
      if (mesg_timer > 0)
        {
          dest.x = (video_width - (images[mesg_img] -> w)) / 2;
          dest.y = status_height;
      ...
      if (level_beat[player] > 0)
        {
          ...
          dest.x = (video_width - (images[IMG_LEVEL_BONUS] -> w)) / 2;
          dest.y = status_height;
          ...
          for (i = 0; i < num_penguinoids && i < (level_beat[player] / 5); i++)
            {
              if (level_beat[player] < 170 - MAX_PENGUINOIDS ||
                ...
                {
                  dest.x = ((video_width - (num_penguinoids * 32)) / 2) + (i * 32);
                  dest.y = status_height + (images[IMG_LEVEL_BONUS] -> h) + 16;
                  dest.w = 32;
                  dest.h = 32;
          ...
          if (num_penguinoids == 0 &&
            ...
            {
              dest.x = (video_width - (images[IMG_NO_BONUS] -> w)) / 2;
              dest.y = status_height + (images[IMG_LEVEL_BONUS] -> h) + 16;
      ...
      if (lives[player] < 0)
        {
          ...
          for (i = 0; i < (images[IMG_GAME_OVER] -> h); i++)
            {
              dest.x = (((video_width - (images[IMG_GAME_OVER] -> w)) / 2) +
                        (my_cos[(i + frame) % 32] * 2));
              dest.y = ((video_height - (images[IMG_GAME_OVER] -> h)) / 2) + i;
...
#ifdef SHOW_FPS
      draw_number(0, (video_height / 2), last_time + (33 * SLOWDOWN) - SDL_GetTicks());
...
int title(void)
{
  ...
  int done, i, j, option, old_option, count, impatience_count, ang,
    angle, angle_m, angle_mm, angle_time, num_starfields, distortion, offset;
  Uint32 last_time;
  SDL_Event event;
  SDLKey key;
  ...
  for (i = 0; i < MAX_STARFIELDS; i++)
    {
      starfields[i].radius = (rand() % (video_width / 2)) + 1;
  ...
  do
    {
      ...
      for (i = 0; i < MAX_STARFIELDS; i++)
        {
          ...
          if (starfields[i].radius >= video_width || starfields[i].time <= 0)
            {
              starfields[i].radius = (rand() % (video_width / 2)) + 1;
              starfields[i].angle = (rand() % 360);
      ...
      for (i = 0; i < num_starfields; i++)
        {
          for (j = 0; j < 30; j++) {
            ...
            dest.x = (cos(M_PI * ang / 180) * (starfields[i].radius - j)) + (video_width / 2);
            dest.y = (-sin(M_PI * ang / 180) * (starfields[i].radius - j)) + (video_height / 2);
            dest.w = 1;
            dest.h = 1;
      ...
      if (distortion < 2)
        {
          dest.x = (video_width - (images[IMG_TITLE_TITLE] -> w)) / 2;
          dest.y = 0;
	  ...
	  offset = ((dest.x + dest.w) / 8) - 10;
          ...
          draw_number((video_width + (images[IMG_TITLE_TITLE] -> w)) / 2 - ((images[IMG_NUMBERS] -> w) / 10) * 6,
...
void setup(void)
{
      ...
      if (i == 0)
        {
          ...
          update_screen( 0, 0, video_width, video_height);
        }
      else if (i >= 1)
        {
          /* Flash "...loading..." text: */
          ...
          if (SDL_GetTicks() > old_ticks + 50)
            {
              ...
              dest.x = (video_width - (images[1] -> w)) / 2;
              dest.y = video_height - (images[1] -> h);
              dest.w = images[1] -> w;
              dest.h = images[1] -> h;
              ...
              update_screen( dest.x, dest.y, dest.w, dest.h);
            }
        }
  ...
  if (use_sound == 1)
    {
      ...
      config_music = Mix_LoadMUS(MUS_CONFIG);
  ...
  /* Reload status area image based on actual video_width: */
  {
    SDL_Surface *status_image;
    const char *status_path;

    if (video_width > 640)
      status_path = DATA_PREFIX "images/status/status-area-800.bmp";
    else
      status_path = DATA_PREFIX "images/status/status-area.bmp";

    status_image = SDL_LoadBMP(status_path);
    if (status_image != NULL)
      {
        SDL_Surface *converted = SDL_DisplayFormat(status_image);
        SDL_FreeSurface(status_image);
        if (converted != NULL)
          {
            /* Free the old status area image */
            SDL_FreeSurface(images[IMG_STATUS_AREA]);
            images[IMG_STATUS_AREA] = converted;
          }
      }

    /* If video dimensions differ from the loaded image, create a scaled version */
    if (images[IMG_STATUS_AREA]->w != video_width ||
        images[IMG_STATUS_AREA]->h != status_height)
      {
        SDL_Surface *scaled_status = SDL_CreateRGBSurface(
          SDL_SWSURFACE, video_width, status_height,
        ...
        if (scaled_status != NULL)
          {
            SDL_Rect src, dest;
            src.x = 0;
            src.y = 0;
            src.w = images[IMG_STATUS_AREA]->w;
            src.h = images[IMG_STATUS_AREA]->h;
            dest.x = 0;
            dest.y = 0;
            dest.w = video_width;
            dest.h = status_height;
            SDL_SoftStretch(images[IMG_STATUS_AREA], &src, scaled_status, &dest);
            SDL_FreeSurface(images[IMG_STATUS_AREA]);
            images[IMG_STATUS_AREA] = scaled_status;
          }
      }
  }
  ...
  do
    {
      ...
      dest.x = 0;
      dest.y = video_height - (images[IMG_PRESS_A_KEY] -> h);
      dest.w = video_width;
      ...
      for (i = (flash % 2); i < images[IMG_PRESS_A_KEY] -> h; i = i + 2)
        {
          dest.x = (((video_width - (images[IMG_PRESS_A_KEY] -> w)) / 2) +
                    my_cos[(i + flash) % 32] * distortion);
          dest.y = video_height - (images[IMG_PRESS_A_KEY] -> h) + i;
      ...
      update_screen( 0, dest.y, video_width, dest.h);
...
{
  SDL_Surface * display;
  int scaled_w = (int)(video_width * video_scale);
  int scaled_h = (int)(video_height * video_scale);

  /* Create the actual display surface at scaled resolution */
  display = SDL_SetVideoMode(scaled_w, scaled_h, 0, flags | SDL_SWSURFACE);
  if (display == NULL)
    return NULL;

  scaled_screen = display;

  if (video_scale == 1.0f)
    {
      /* No scaling needed, render directly to display */
      return display;
    }
  else
    {
      /* Create an off-screen render surface at base resolution */
      return SDL_CreateRGBSurface(SDL_SWSURFACE, video_width, video_height,
...
void flip_screen(void)
{
  if (video_scale == 1.0f)
  ...
  else
    {
      ...
      dest.x = 0;
      dest.y = 0;
      dest.w = scaled_screen->w;
      dest.h = scaled_screen->h;
      SDL_SoftStretch(screen, &src, scaled_screen, &dest);
      SDL_Flip(scaled_screen);
    }
}


/* Update a region of the screen (handles scaling) */
...
void update_screen(int x, int y, int w, int h)
{
  if (video_scale == 1.0f)
    ...
    {
      /* With scaling, do a full screen update */
      SDL_Rect src, dest;
      src.x = 0;
      src.y = 0;
      src.w = screen->w;
      ...
      dest.h = scaled_screen->h;
      SDL_SoftStretch(screen, &src, scaled_screen, &dest);
      SDL_UpdateRect(scaled_screen, 0, 0, scaled_screen->w, scaled_screen->h);
    }
}
...
void my_shutdown(void)
{
  ...
  /* Free the render surface if we created a separate one for scaling */
  if (video_scale != 1.0f && screen != NULL)
    {
      ...
      if (i < num)
        {
          ...
          penguinoids[ply][i].y = video_height - tile_height;
...
void add_alien(int x, int y, int type, int mode, int timer)
{
  ...
  if (y == -1)
    y = (rand() % (video_height / 2)) + status_height;
  ...
  for (i = 0; i < MAX_STARS; i++)
    {
      ...
      stars[i].y = (rand() % (video_height - status_height)) + status_height;
    }
}
...
{
  int i, y;
  int play_height = video_height - status_height;
  int aurora_min = status_height + play_height * 5 / 100;
  int aurora_max = status_height + play_height * 40 / 100;
  int aurora_start = status_height + play_height * 10 / 100;
  int aurora_range = play_height * 25 / 100;

  y = (rand() % aurora_range) + aurora_start;
  ...
  for (ax = 0; ax < video_width; ax++)
    {
      ...
      while (dx >= video_width)
        dx = dx - video_width;

      for (y = status_height; y < auroras[ax]; y++)
        {
          lum = 80 + (auroras[ax] - y);
...
void kill_alien(int i)
{
  ...
  if (aliens[player][i].type != ALIEN_BOSS ||
      aliens[player][i].y >= video_height)
    {
      aliens[player][i].alive = 0;
          ...
          if (aliens[player][i].alive == 1)
            {
              ...
              if (tmp_x >= -32 && tmp_x <= (video_width + 32))
                {
                  /* Kill the alien: */
          ...
          if (bullets[i].alive == 1)
            {
              ...
              if (tmp_x >= -64 && tmp_x <= (video_width + 32))
                {
                  /* Kill the alien: */
...
void usage(int err)
{
  ...
  fprintf(fi, "\n"
          "Usage: defendguin [--disable-sound] [--fullscreen] [--width N]\n"
          "       [--height N] [--scale N] |\n"
          ...
          "                     (Also '-f')\n"
          "  --width N          Set video width (default: %d). (Also '-w')\n"
          "  --height N         Set video height (default: %d). (Also '-h')\n"
          "  --scale N          Set video scale, e.g. 1.5 (default: 1.0). (Also '-s')\n"
          "\n"
...
void pause_screen(void)
{
  ...
  x = (rand() % (video_width - 32));
  y = (rand() % (video_height - 32));
  xm = (rand() % 20) - 10;
  ym = (rand() % 20) - 10;
  ...
  do
    {
      ...
      if (mode == 0)
        {
          for (y = (frame % 2); y < video_height; y = y + 2)
            {
        ...
        {
          for (r = 0; r < (video_height / 2); r = r + 5)
            {
              for (a = 0; a < 360; a = a + 2)
                {
                  src.x = (video_width / 2) + cos(M_PI * a / 180) * r;
                  src.y = (video_height / 2) - sin(M_PI * a / 180) * r;
                  src.w = 8;
                  src.h = 8;
                  
                  dest.x = (video_width / 2) + cos(M_PI * (a + (frame * 5)) / 180) * r;
                  dest.y = (video_height / 2) - sin(M_PI * (a + (frame * 5)) / 180) * r;
                  dest.w = 8;
                  dest.h = 8;
        ...
        {
          for (r = 0; r < (video_height / 2); r = r + 10)
            {
              for (a = 0; a < 360; a = a + 5)
                {
                  src.x = (video_width / 2) + cos(M_PI * a / 180) * r;
                  ...
                  dest.x = (video_width / 2) + cos(M_PI * (a + ((frame % 3600) * r / 100)) /
                                     180) * r;
                  dest.y = (video_height / 2) - sin(M_PI * (a + ((frame % 3600) * r / 100)) /
                                     180) * r;
                  dest.w = 10;
                  dest.h = 10;
...
void write_text(int x, int y, char * str)
{
  ...
  update_screen( x * 8, y * 10, strlen(str) * 8, 8);
}
